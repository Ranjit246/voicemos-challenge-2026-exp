"""
Idea K (v2) — richer-space RAMP.

Cheap RAMP retrieved in the 35-d cosine space where ridge already generalizes,
so it tied. This version retrieves in the HIGH-DIM SSL pair-embedding-difference
space, where the representation->score map is genuinely brittle and kNN can beat
the parametric fit (RAMP's actual mechanism).

Pair retrieval vector = concat of L2-normalized embedding DIFFERENCES from a panel
(ECAPA, WavLM early layer 3 = speaker, WavLM mid layer 12 = accent, CommonAccent),
standardized, PCA-reduced. kNN over that; fuse with the ridge parametric.

Validation: sklearn GroupKFold-by-system (calibrated). Compares ridge vs
richer-retrieval vs fusion. --submit builds submission-2/answer.txt.

Usage:
    python richer_ramp.py [--submit]
"""

import argparse, csv, glob, os, sys
import numpy as np
import pandas as pd
import torch
from scipy.stats import spearmanr, zscore
from scipy.spatial.distance import cdist
from sklearn.model_selection import GroupKFold
from sklearn.linear_model import Ridge
from sklearn.preprocessing import StandardScaler
from sklearn.decomposition import PCA
from sklearn.pipeline import make_pipeline

BASE = os.path.dirname(os.path.abspath(__file__)); EXP = os.path.dirname(BASE)
sys.path.insert(0, os.path.join(EXP, "idea-e-feature-fusion"))
from features import build, train_table, emb_key  # noqa: E402

DEV_CSV = "/Users/ranjitpatro/Home/Research/VoiceMOS/dataset/vmc2026_track3_train_phase_distro_v3_syn/sets/dev.csv"
N_SPLITS = 7; K = 30; ALPHA = {"spk_sim": 0.5, "acc_sim": 0.6}; PCA_DIM = 128
OOF = os.path.join(EXP, "idea-i-ensemble/oof")


def srcc(a, b): return spearmanr(a, b).statistic


def load(p):
    d = torch.load(p, weights_only=False)
    return {k: (v.numpy() if isinstance(v, torch.Tensor) else np.asarray(v)) for k, v in d.items()}


def norm(v): return v.astype(np.float32) / (np.linalg.norm(v) + 1e-8)


def pair_diff_matrix(df):
    """Rich retrieval vector per pair: concat of L2-normalized embedding differences."""
    ecapa = load(os.path.join(EXP, "ecapa/embeddings.pt"))
    ca = load(os.path.join(EXP, "idea-e-feature-fusion/commonaccent_emb.pt"))
    wavlm = load(os.path.join(EXP, "idea-f-wavlm/wavlm_layers.pt"))
    rows = []
    for a, b in zip(df.wav_a_path, df.wav_b_path):
        ka, kb = emb_key(a), emb_key(b)
        parts = [
            norm(ecapa[ka]) - norm(ecapa[kb]),
            norm(ca[ka]) - norm(ca[kb]),
            norm(wavlm[ka][3]) - norm(wavlm[kb][3]),     # early = speaker
            norm(wavlm[ka][12]) - norm(wavlm[kb][12]),   # mid = accent
        ]
        rows.append(np.concatenate(parts))
    return np.stack(rows)


def wl_cosfeats(df):
    wavlm = load(os.path.join(EXP, "idea-f-wavlm/wavlm_layers.pt"))
    n = next(iter(wavlm.values())).shape[0]
    for L in range(n):
        df[f"wl_{L}"] = [float(np.dot(a := wavlm[emb_key(p)][L].astype(np.float32),
                                      b := wavlm[emb_key(q)][L].astype(np.float32)) /
                              (np.linalg.norm(a)*np.linalg.norm(b)+1e-8))
                        for p, q in zip(df.wav_a_path, df.wav_b_path)]
    return [f"wl_{L}" for L in range(n)]


def retrieve(Xtr, ytr, Xq, k=K):
    D = cdist(Xq, Xtr)
    out = np.empty(len(Xq))
    for i in range(len(Xq)):
        nn = np.argpartition(D[i], k)[:k]; d = D[i, nn]
        w = np.exp(-d / (d.mean() + 1e-8)); w /= w.sum()
        out[i] = (w * ytr[nn]).sum()
    return out


def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--submit", action="store_true")
    args = ap.parse_args()

    train, base = train_table(); train = train.reset_index(drop=True)
    dev, _ = build(pd.read_csv(DEV_CSV))
    wl = wl_cosfeats(train); wl_cosfeats(dev)
    feats = base + wl
    print("building rich pair-difference retrieval space...")
    Rtr = pair_diff_matrix(train); Rdev = pair_diff_matrix(dev)
    print(f"  retrieval dim (raw) = {Rtr.shape[1]}  -> PCA {PCA_DIM}")
    groups = train.system_id.values

    betas = np.linspace(0, 1, 11)
    print(f"\ngrouped-CV (calibrated). K={K}")
    print(f"{'target':>8} {'ridge':>7} {'rich_retr':>9} {'best_fuse':>9} {'beta*':>6}")
    best_beta = {}
    for t in ["spk_sim", "acc_sim"]:
        gkf = GroupKFold(n_splits=N_SPLITS)
        rid = np.zeros(len(train)); ret = np.zeros(len(train))
        X = train[feats].values; y = train[t].values
        for tr, te in gkf.split(train, groups=groups):
            sc = StandardScaler().fit(X[tr]); m = Ridge(alpha=1.0).fit(sc.transform(X[tr]), y[tr])
            rid[te] = m.predict(sc.transform(X[te]))
            # rich retrieval space: standardize + PCA fit on train fold
            rs = StandardScaler().fit(Rtr[tr])
            pca = PCA(n_components=PCA_DIM, random_state=0).fit(rs.transform(Rtr[tr]))
            Ztr, Zte = pca.transform(rs.transform(Rtr[tr])), pca.transform(rs.transform(Rtr[te]))
            ret[te] = retrieve(Ztr, y[tr], Zte)
        r_s, t_s = srcc(y, rid), srcc(y, ret)
        fuse = [srcc(y, b*zscore(rid) + (1-b)*zscore(ret)) for b in betas]
        bi = int(np.argmax(fuse)); best_beta[t] = betas[bi]
        print(f"{t:>8} {r_s:>7.4f} {t_s:>9.4f} {fuse[bi]:>9.4f} {betas[bi]:>6.1f}")

    print("\n(cheap cosine-space RAMP was: spk fuse 0.6483 / acc 0.6017; ridge 0.6437 / 0.5978)")
    if not args.submit:
        return

    head_dev = None
    fs = sorted(glob.glob(os.path.join(OOF, "head_s*.npz")))
    if fs:
        head_dev = np.mean([np.stack([np.load(f)["dev_spk"], np.load(f)["dev_acc"]], 1) for f in fs], 0)

    out = os.path.join(BASE, "submission-2"); os.makedirs(out, exist_ok=True)
    cols = {}
    X = train[feats].values; Xd = dev[feats].values
    rs = StandardScaler().fit(Rtr); pca = PCA(n_components=PCA_DIM, random_state=0).fit(rs.transform(Rtr))
    Ztr, Zdev = pca.transform(rs.transform(Rtr)), pca.transform(rs.transform(Rdev))
    for j, t in enumerate(["spk_sim", "acc_sim"]):
        sc = StandardScaler().fit(X); y = train[t].values
        rid = Ridge(alpha=1.0).fit(sc.transform(X), y).predict(sc.transform(Xd))
        ret = retrieve(Ztr, y, Zdev)
        b = best_beta[t]; ramp = b*zscore(rid) + (1-b)*zscore(ret)
        parts = [zscore(ramp)]
        if head_dev is not None: parts.append(zscore(head_dev[:, j]))
        param = np.mean(parts, 0)
        prior = dev.system_id.map(train.groupby("system_id")[t].mean()).values.astype(float)
        seen = ~np.isnan(prior); zp = np.empty(len(dev))
        zp[seen] = zscore(prior[seen]); zp[~seen] = zscore(param)[~seen]
        a = ALPHA[t]; v = a*zscore(param) + (1-a)*zp
        cols[t] = 1 + 4*(v - v.min())/(v.max()-v.min()+1e-8)

    with open(os.path.join(out, "answer.txt"), "w", newline="") as fh:
        wr = csv.writer(fh); wr.writerow(["system_id","utterance_id","wav_a_path","wav_b_path","pred_acc_sim","pred_spk_sim"])
        for i, r in dev.iterrows():
            wr.writerow([r.system_id, r.utterance_id, r.wav_a_path, r.wav_b_path,
                         float(cols["acc_sim"][i]), float(cols["spk_sim"][i])])
    print(f"Saved -> {out}/answer.txt")


if __name__ == "__main__":
    main()
